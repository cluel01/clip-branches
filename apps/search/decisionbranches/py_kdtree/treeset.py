...
from .kdtree_int import KDTree as KDTreeInt
...
class KDTreeSet():
    def __init__(self,indexes,path=None,dtype="float64",model_name="tree.pkl",verbose=True,group_prefix="",**kwargs):       
        ...
        for i in indexes:
            ...
            if self.dtype == "uint8":
                tree = KDTreeInt(path=full,dtype=dtype,model_file=model_name,verbose=self.verbose,**kwargs)
            else:
    ...
    '''
    def fit(self,X,mmap=False,build_inmemory=False):    
        for i in self.indexes:
            ...
            if self.trees[dname].tree is not None:
                ...
                if not mmap:
                    self.trees[dname].fit(X[:,i],build_inmemory=build_inmemory)
                else:
                    self.trees[dname].fit(X,mmap_idxs=i,build_inmemory=build_inmemory)
    ...
    def multi_query_nn_cy(self,query_point,k=5,idxs=None,stop_leaves=None):
        if isinstance(query_point,np.ndarray):
            if query_point.dtype != np.dtype(self.dtype):
                query_point = query_point.astype(self.dtype)
        ...
        loaded_leaves = 0
        for i in idxs:
            dname = "_".join([self.group_prefix + str(j) for j in sorted(i)])
            query = query_point[i]
            ix,_,_,leaves = self.trees[dname].query_point_cy(query,k=k,stop_leaves=stop_leaves)
            loaded_leaves += leaves
            inds.append(ix)
