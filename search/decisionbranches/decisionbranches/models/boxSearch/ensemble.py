...
class Ensemble():
    def __init__(self,tot_feat,n_estimators=10,n_feat=5,n_ind=1000,max_feat="all",indices=None,n_jobs=5,max_samples=None,bootstrap=True,
                    cfg=None,feats=None,feat_cfg=None,postTree=False, verbose=True,debug=False,dtype="float64",dtype_int = "int32",seed=42):
        '''
        ...
        if indices is not None:
            if isinstance(indices,list):
                if len(set([len(x) for x in indices])) != 1:
                    ...
                    indices = np.array([lst + [-1]*(max_length - len(lst)) for lst in indices],dtype=dtype_int)
                    indices = np.ma.masked_equal(indices, -1)
                    self.masked = True
                else:
    ...
    def fit(self,x,y,pos_label=1):
        ...
        for i in range(self.n_estimators):
            ...
            se.set_feat_idxs(self.feat_idxs,self.tot_feat,masked=self.masked)
            if self.bootstrap:
        ...
        if self.verbose:
            if self.masked:
                print("Number of covered features: ",len(np.unique(self.feat_idxs.compressed())))
            else:
    ...
    def get_boxes_uint8(self):
        mins, maxs, fidxs = self.get_boxes()
